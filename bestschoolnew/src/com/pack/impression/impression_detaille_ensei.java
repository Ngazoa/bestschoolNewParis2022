 ...
 * @author Benito
 */
public class impression_detaille_ensei extends JPanel implements Printable {

    List<impressionmodel_11> eleves = new ArrayList<impressionmodel_11>();
    int comp = 29;

    public impression_detaille_ensei(List<impressionmodel_11> eleve) {
        eleves = eleve;

        PrinterJob pj = PrinterJob.getPrinterJob();
        pj.setPrintable(impression_detaille_ensei.this);
        pj.printDialog();
        try {
            pj.setJobName("LISTE DES ENSEIGNANTS");
    ...
    public int print(Graphics g, PageFormat pageFormat, int pageIndex) throws PrinterException {
        ...
        g2.drawString("Liste enseignants de l'établissement scolaire ", 115, 190);
        g2.setFont(new Font(Font.MONOSPACED, Font.BOLD, 10));
        ...
        g2.drawString("Classe occupée", positionX + 200 + 10, positionY);
        g2.drawString("Matiere ensei.", positionX + 340 + 10, positionY);
        g2.drawString("Contact telphon.", positionX + 440 + 15, positionY);
        ...
        for (int j = pageIndex * comp; j < eleves.size() && j < comp * (pageIndex + 1); j++) {
            ...
            g2.drawString(eleves.get(j).lnaissance(), positionX + 200 + 10, positionY);
            g2.drawString(nommatiere(eleves.get(j).sexe()), positionX + 340 + 10, positionY);
            g2.drawString(eleves.get(j).moyenne(), positionX + 440 + 15, positionY);
            i = i + 1;
    ...
    private String nommatiere(String name) {
